...
def curve_fit(points, error: float, corner_angle: float | None, is_cyclic: bool):
    if corner_angle is None:
        corner_angle = DEFAULT_CORNER_ANGLE
    return curve_fit_nd.curve_from_points(points, error, corner_angle, is_cyclic)


def curve_error_max(points, curve) -> tuple[float, list]:
    """Return (max_error, measure_points) for visualization."""
    error_max_sq = 0.0
    measure_points = []
    for (i0, v0), (i1, v1) in pairwise(curve):
        k0 = v0[1]
        h0 = v0[2]
        ...
        lens = [None] * (i1 - i0)
        lens_accum = 0.0
        for i, (s0, s1) in enumerate(pairwise(range(i0, i1 + 1))):
            lens[i] = lens_accum
...
def test_data_load(name: str) -> Sequence[tuple[float, float]]:
    return import_module(name).data
...
class TestDataFile_MixIn:
    ...
    def assertTestData(
        ...
        error_test, measure_points = curve_error_max(points, curve)
...
class FreehandTest(unittest.TestCase, TestDataFile_MixIn):
    pass
